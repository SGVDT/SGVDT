...
var dailyData = function(callback) {
  request.get('https://data.seattle.gov/resource/tn4m-tpqu.json')
    .end((err, res) => {
      ...
      // process.stdout.write('res body length: ' + res.body.length);

      for (var i = 0; i < res.body.length; i++) {
        Offense.update( { rms_cdw_id: res.body[i].rms_cdw_id }, {
          ...
          rms_cdw_id: res.body[i].rms_cdw_id }, { upsert: true }, (err) => {
            if (err) {
              return eH(err);
            }
          });
      ...
      // console.log('updated in dailyData function');
      callback();
    });
};
